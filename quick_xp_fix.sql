-- Quick XP Fix - Drop and recreate function cleanly
-- Run this if the main fix file gives errors

-- 1. Drop existing function completely
-- ===================================================================
DROP FUNCTION IF EXISTS calculate_user_xp_basic(UUID);
DROP FUNCTION IF EXISTS calculate_user_xp_basic;

-- 2. Create new calculate_user_xp_basic function
-- ===================================================================
...
BEGIN
    ...
    VALUES (target_user_id, username_text, total_xp, NOW())
    ON CONFLICT (user_id)
    DO UPDATE SET
...
DO $$
BEGIN
    RAISE NOTICE 'Function calculate_user_xp_basic recreated successfully!';
    RAISE NOTICE 'You can now test XP calculation.';
END $$;
